...
```mermaid
flowchart LR
    A["👨‍🏫 Profesor"] -- "Crea asignaturas<br/>y proyectos" --> B["📚 PaaSify"]
    C["🎓 Alumno"] -- "Sube código o<br/>elige imagen" --> B
    B -- "Docker SDK" --> D["� Docker Engine"]
    D -- "Contenedor activo<br/>+ puerto público" --> E["🌐 Web"]
```

> 📖 Diagramas técnicos detallados (arquitectura, flujos internos, ciclo de vida Docker): [docs/DEVELOPMENT.md](docs/DEVELOPMENT.md)
