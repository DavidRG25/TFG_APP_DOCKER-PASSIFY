...
4. Sincronizar el estado real del contenedor para detectar fallos de arranque (p.ej., MySQL sin contraseña).
...
3. **Media / user_code**
   ...
   - Se eliminó el directorio raíz `user_code/` y los ZIP legacy, dejando solo `media/user_code`.
4. **Sincronización de estado**
   - `_sync_service` revisa `container.status` en Docker y, si detecta que un servicio marcado como `running/pending` terminó (`exited/dead`), actualiza el `Service` a `error` con los logs correspondientes.
...
3. Storage: Al subir ZIP/Dockerfile, los archivos aparecen únicamente en `media/user_code/...` y no se recrea un directorio `user_code` en la raíz del repo.
4. Estado real: Probar una imagen que falle en arranque (ej. `mysql:latest` sin contraseña) y confirmar que el servicio pasa a `ERROR` mostrando los logs.
