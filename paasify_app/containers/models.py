        ...
        if self.pk:
            try:
                ...
                for field_name in ['dockerfile', 'compose', 'code']:
                    ...
                    # Si antes había un archivo y ahora es distinto o se ha borrado
                    if old_file and old_file != new_file:
                        try:
                            # Importante: Solo borrar si el archivo físico es realmente distinto
                            # o si el nuevo es directamente None.
                            if not new_file or old_file.path != new_file.path:
                                if os.path.exists(old_file.path):
                                    os.remove(old_file.path)
                                    print(f"[cleanup] Borrado archivo físico antiguo de {field_name}")
                        except Exception as e:
                            print(f"[warning] Error limpiando archivo {field_name}: {e}")
