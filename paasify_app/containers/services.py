...
def prepare_service_workspace(service: Service, *, unpack_code: bool = True, keep_volumes: bool = True) -> Path:
    ...
    if not keep_volumes:
        import shutil
        # Archivos que NUNCA debemos borrar porque son la definición del servicio.
        # Protegemos basándonos en los ARCHIVOS QUE TIENE ASIGNADOS el objeto Service.
        master_basenames = []
        if service.dockerfile: master_basenames.append(os.path.basename(service.dockerfile.path))
        if service.compose: master_basenames.append(os.path.basename(service.compose.path))
        if service.code: master_basenames.append(os.path.basename(service.code.path))
        
        # También protegemos los nombres estándar que vamos a generar luego 
        # para que no se borren en este paso si ya existían.
        master_basenames += ["Dockerfile", "docker-compose.yml", "source.zip", "source.rar"]
        
        print(f"[prepare_service_workspace] Limpieza 'Nuclear' para servicio {service.id}. Protegiendo: {master_basenames}")
        for item in workspace.iterdir():
            try:
                if item.is_file():
                    if item.name not in master_basenames:
                        item.unlink()
                elif item.is_dir():
                    # Borrar TODAS las carpetas (web, app, .git, etc)
                    shutil.rmtree(item)
            except Exception as e:
                print(f"[warning] No se pudo eliminar residual {item}: {e}")
    ...
    if unpack_code and service.code:
        try:
            ...
            # Copiar archivo solo si existe físicamente
            if not os.path.exists(service.code.path):
                print(f"[warning] El archivo de código no existe en disco: {service.code.path}")
                return workspace

            shutil.copy2(service.code.path, dest_archive)
            
            # Extraer
            _unpack_code_archive_to(str(workspace), archive_path=str(dest_archive))
            
            # Limpiar rastro tras extracción
            for item in workspace.iterdir():
                if item.is_file() and item.name not in ["Dockerfile", "docker-compose.yml", "source.zip", "source.rar"]:
                    if "_" in item.name: # Archivos temporales de Django con sufijos
                        try: item.unlink()
                        except: pass
            
            # Borrar el ZIP del contexto de build si no queremos persistencia
            if not keep_volumes:
                try: dest_archive.unlink()
                except: pass
...
def _run_command_stream_logs(service: Service, cmd: list[str], cwd: str = None):
    ...
    try:
        ...
        if process.returncode != 0:
            # Recuperar últimas líneas de error para el mensaje de excepción
            error_tail = (service.logs or "").split("\n")[-20:]
            error_summary = "\n".join(error_tail)
            raise RuntimeError(f"El comando falló con código {process.returncode}. Detalles:\n{error_summary}")
...
def _run_compose_service(service: Service, docker_client, force_restart: bool, container_configs: dict | None = None):
    ...
    try:
        ...
        # --- GESTIÓN DE PERSISTENCIA (keep_volumes) ---
        project = _get_compose_project_name(service)
        keep_vols = getattr(service, "_keep_volumes", True)
        
        if not keep_vols:
            _append_log(service, "Persistencia desactivada: Eliminando volúmenes previos...")
            try:
                # Correr down -v para limpiar volúmenes de este proyecto
                down_cmd = _compose_cmd() + ["-p", project, "-f", str(compose_path), "down", "-v", "--remove-orphans"]
                subprocess.run(down_cmd, cwd=str(workspace), capture_output=True, check=False)
                _append_log(service, "Volúmenes eliminados.")
            except Exception as e:
                _append_log(service, f"Nota: No se pudieron limpiar volúmenes previos: {e}")
...
def _run_simple_service(service: Service, docker_client, force_restart: bool, custom_port: int | None, command: list[str] | None):
    ...
    try:
        ...
        # Preparar workspace pasando el flag explícito
        workspace = prepare_service_workspace(service, keep_volumes=keep_volumes)
        ...
        if image_vols:
            for i, vol_path in enumerate(image_vols.keys()):
                ...
                # Si no hay persistencia, borrar el volumen antes de empezar
                if not keep_volumes:
                    try:
                        v = docker_client.volumes.get(vol_name)
                        v.remove(force=True)
                        _append_log(service, f"Limpio volumen de datos: {vol_name}")
                    except NotFound:
                        pass
...
def _run_container_worker(service_id: int, force_restart: bool, custom_port: int | None, command: list[str] | None, container_configs: dict | None = None, keep_volumes: bool = True) -> None:
    ...
    try:
        ...
        # Inyectar el flag para que las funciones internas lo vean
        service._keep_volumes = keep_volumes
...
def run_container(service: Service, force_restart: bool = False, custom_port: int | None = None, command: list[str] | None = None, container_configs: dict | None = None, keep_volumes: bool = True):
    """Interfaz publica para lanzar la ejecucion (sincrona o asincrona)."""
    ...
    service.save(update_fields=["status", "logs", "updated_at"])
    EXECUTOR.submit(_run_container_worker, service.pk, force_restart, custom_port, command, container_configs, keep_volumes)
