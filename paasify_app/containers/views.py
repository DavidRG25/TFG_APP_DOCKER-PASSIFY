...
class ServiceViewSet(viewsets.ModelViewSet):
    ...
    def perform_update(self, serializer):
        ...
        def _restart_task():
            try:
                ...
                from .services import remove_container, run_container
                remove_container(updated_service, keep_files=True, keep_volumes=keep_vols)
                run_container(updated_service, keep_volumes=keep_vols)
            except Exception as exc:
                print(f"[ERROR] Error diferido al reiniciar: {exc}")
    ...
    def _attach_uploaded_files(self, service: Service, request, mode: str):
        """
        Sincroniza los archivos físicos con la base de datos según el modo y la persistencia.
        Asegura que 'No preservar datos' limpie el código residual pero mantenga la definición.
        """
        if mode == "default":
            return
            
        keep_vols = getattr(service, "_keep_volumes", True)
        
        # 1. Identificar archivos subidos en esta petición
        up_compose = request.FILES.get('compose')
        up_dockerfile = request.FILES.get('dockerfile')
        up_code = request.FILES.get('code')
        
        # 2. Lógica de Integridad de Modo y Limpieza (Factory Reset)
        # Si sube un tipo, el otro debe desaparecer.
        if up_compose:
            service.dockerfile = None
        elif up_dockerfile:
            service.compose = None
        
        # 3. Gestión de la carpeta de código (ZIP/RAR)
        if up_code:
            # Reemplazo por nuevo código
            service.code.save(up_code.name, up_code, save=False)
        elif not keep_vols:
            # SI NO hay código nuevo Y NO queremos preservar datos -> Borramos el código anterior
            # Esto garantiza que el despliegue sea 'limpio' de fuentes viejos.
            if service.code:
                # Borrado físico forzado inmediato para evitar problemas de sincronía "fantasma"
                import os
                try:
                    if os.path.exists(service.code.path):
                        os.remove(service.code.path)
                        print(f"[clean-install] Borrado físico inmediato de código: {service.code.path}")
                except Exception as e:
                    pass
                
                print(f"[clean-install] Purgando código residual en base de datos: {service.code}")
                # Forzar borrado del campo en Django FileField
                service.code.delete(save=False)
                service.code = None

        # 4. Gestión de archivos Maestros (SOLO si se han subido nuevos)
        # IMPORTANTE: No ponemos a None si no se suben, o romperíamos el servicio (Factory Reset de datos != Borrar servicio)
        if up_compose:
            service.compose.save(up_compose.name, up_compose, save=False)
        if up_dockerfile:
            service.dockerfile.save(up_dockerfile.name, up_dockerfile, save=False)
            
        # 5. Guardar cambios en DB para disparar el cleanup físico del modelo
        service.save()
...
def edit_service(request, pk):
    ...
    if request.method == "POST":
        ...
        # Parsear keep_volumes: el hidden input envía "false", el checkbox envía "true"
        keep_vols_raw = request.POST.get("keep_volumes", "true")
        keep_vols = keep_vols_raw.lower() not in ("false", "0", "off", "no")
        print(f"[edit_service] keep_volumes raw='{keep_vols_raw}' -> keep_vols={keep_vols}")
        ...
        # === FACTORY RESET: Si no preservamos datos, limpiar código viejo ===
        if not keep_vols:
            # 1. Borrar campo code de la DB si no se ha subido uno nuevo
            if not request.FILES.get("code") and service.code:
                try:
                    if hasattr(service.code, 'path') and os.path.exists(service.code.path):
                        os.remove(service.code.path)
                        print(f"[factory-reset] Borrado físico del ZIP: {service.code.path}")
                except Exception:
                    pass
                service.code = None
            
            # 2. Limpiar TODA la carpeta del servicio (excepto archivos maestros)
            workspace = Path(settings.MEDIA_ROOT) / "services" / str(service.pk)
            if workspace.exists():
                # Proteger solo: compose actual, dockerfile actual, y archivos recién subidos
                protected = {"Dockerfile", "docker-compose.yml"}
                if service.compose:
                    protected.add(os.path.basename(service.compose.name))
                if service.dockerfile:
                    protected.add(os.path.basename(service.dockerfile.name))
                if service.code:
                    protected.add(os.path.basename(service.code.name))
                
                for item in workspace.iterdir():
                    try:
                        if item.is_dir():
                            shutil.rmtree(item)
                            print(f"[factory-reset] Borrada carpeta: {item.name}")
                        elif item.is_file() and item.name not in protected:
                            item.unlink()
                            print(f"[factory-reset] Borrado archivo: {item.name}")
                    except Exception as e:
                        print(f"[factory-reset] Warning al borrar {item}: {e}")

        service.status = "updating"
        service.save()
        # Refrescar desde DB para que las rutas de archivos (con sufijos de Django) estén actualizadas
        ...
        try:
            ...
            run_container(service, container_configs=configs_to_use, custom_port=new_assigned_port, keep_volumes=keep_vols)
        except Exception as exc:
