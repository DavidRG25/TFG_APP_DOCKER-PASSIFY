...
- **Acceso Dual (DNS / Puerto)**: 
  - Usa el icono del **Mundo (DNS)** para acceder por el subdominio dinámico (Traefik).
  - Usa el icono de **Red (Puerto)** si tienes problemas de cabeceras en tu servidor Nginx o necesitas acceso directo por IP:Puerto.
- **Nota sobre Bases de Datos**: El acceso por subdominio (DNS) para **PostgreSQL/Redis** en el navegador se quedará cargando, ya que no son protocolos HTTP. Úsalos solo por red interna o clientes de DB.
...
_Ejemplo optimizado para Dual Access en PaaSify_
_Realizado para validación de TFG - PaaSify Cloud Platform_
